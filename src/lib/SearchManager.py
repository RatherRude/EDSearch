...
class SearchManager:
    ...
    def route_sources(
        self, query: str, context: list[dict], entities: list[dict]
    ) -> dict[str, bool]:
        search_categories_schema: dict = {
            "type": "object",
            "properties": {},
            "required": [],
        ...
        for source in self.data_sources:
            search_categories_schema["properties"][source["name"]] = {
                "type": "boolean",
    ...
    def run(self, query: str, context: list[dict], state: dict[str, dict]):
        ...
        for source in self.data_sources:
            if categories.get(source["name"], False):
                results["results"][source["name"]] = source["method"](
                    query, context, state, found_entities
                )
            # else:
